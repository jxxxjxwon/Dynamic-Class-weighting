...
반도체 제조 공정에서는 미세한 결함이 수율에 큰 영향을 미치며, 이를 분석하기 위해 사용되는 웨이퍼맵 데이터는 클래스 불균형 문제가 두드러진다. 본 연구는 이러한 문제를 해결하기 위해 경량 CNN 모델에 동적 가중치 조정 기법을 결합한 분류 방식을 제안한다. 경량화된 모델을 사용함으로써 복잡도를 크게 높이지 않으면서도, Epoch마다 Recall 기반 가중치를 재계산해 적용하여 소수 클래스의 분류 성능을 기존 방법론 대비 효율적으로 개선하였다.
...
Metric별 동적 가중치 모델의 효과를 검증하기 위해 Recall, F1 Score, Accuracy를 기준으로 각각 동적 가중치를 산출한 세 가지 모델을 학습하였다. 모든 모델은 동일한 환경에서 동일한 데이터셋으로 학습과 검증을 수행했으며, 매 Epoch에서 산출된 지표의 역수를 기반으로 클래스별 가중치를 재조정하는 방식으로 성능을 비교하였다.
...
**Baseline Model**은 어떠한 가중치 조정 기법도 적용하지 않은 파라미터 0.3M의 경량 CNN 구조로, 본 연구에서 사용하는 기본 성능 기준점으로 활용하였다.  
...
**동적 가중치 조절 Model**은 본 연구에서 검증하고자 하는 모델이며, 1번 실험 결과를 적용하여 Metric를 Recall로 적용하였다.
